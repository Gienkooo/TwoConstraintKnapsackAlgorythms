...
std::vector<std::vector<int>> Genetic::populate(){
    ...
    sort(population.begin(), population.end(), [this](std::vector<int> &v1, std::vector<int> &v2) {return problem->fitness(v1) > problem->fitness(v2);});
    //displayPopulation(population, 2137);
    return population;
}
...
void Genetic::evolvePopulation(std::vector<std::vector<int>> &populationPrev){
    ...
    //sort(populationNext.begin(), populationNext.end(), [this](std::vector<int> v1, std::vector<int> v2) {return problem->fitness(v1) > problem->fitness(v2);});
    ...
    sort(populationNext.begin(), populationNext.end(), [this](std::vector<int> v1, std::vector<int> v2) {return problem->fitness(v1) > problem->fitness(v2);});
    populationNext.erase(populationNext.begin() + populationSize, populationNext.end());
    populationPrev = populationNext;
}

void Genetic::displayPopulation(std::vector<std::vector<int>> population, int generationNo){
